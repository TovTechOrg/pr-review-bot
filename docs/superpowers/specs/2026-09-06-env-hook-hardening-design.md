...
Trust & Safety / secret-exposure incidents it documents, `bot/config.py`
(`SettingsConfigDict(env_file=...)`), `bot/scripts/_override.py` and
`bot/providers/credentials.py` (both already use `dotenv_values()` directly —
the pattern this design reuses).

## 1. Problem and context

The current `PreToolUse` hook (`check_env_access.py`) protects `bot/.env` by
...
- **False negatives.** A command that never types the string `.env` — `grep
  -rn foo bot/`, `find bot -exec cat {} +`, a Python one-liner reading the
  ...
  *text*. This already happened for real: `ISSUES.md` documents a
  ...
  `bot/.env` and printed 3 (non-secret) lines from it, undetected by the
...
- **`chmod 000` on `bot/.env` itself** was considered and rejected: `bot/config.py`'s
  ...
  real `Settings()`) reads the file in-process, and `bot/scripts/_override.py`
  / `bot/providers/credentials.py` already call `dotenv_values()` on it
...
2. Call `dotenv_values(env_path)` on `bot/.env`, resolved relative to the
...
- **`.claude/hooks/redact_output.py`** (new) — the filter process described
  above. Takes no arguments; reads stdin, writes stdout, resolves `bot/.env`
  ...
  `python-dotenv` is available in that context (it's already a `bot`
  dependency; `--no-project` must still resolve it, or the filter needs its
...
- `bot/.env` missing entirely (e.g. a fresh checkout before setup) → treated
