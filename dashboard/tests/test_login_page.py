...
async def test_login_page_language_options_are_plain_text_no_icon():
    """A per-language icon (first flag emoji, then an authored SVG flag
    substitute) was tried and reverted -- both onboarding wizard and this
    dashboard settled on plain "English"/"עברית" with no icon at all,
    simpler and consistent across both projects."""
    client = await _client()
    body = (await client.get("/login")).text
    lang_section = body[
