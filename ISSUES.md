...
- **What:** The original bug (the cookie-less bypass fired for every cookie-less request, including a normal browser's very first visit, so the login screen never rendered for anyone) was fixed by gating the bypass on `?cookieless=1` plus "no cookies present." But the query param only arrives on the top-level navigation; every dashboard XHR (`/api/dashboard`, etc.) is a bare-path fetch with no `?cookieless=1`, so a genuinely cookie-hostile browser gets `200` on the page load, `401` on the first XHR, a client-side redirect to `/login`, and `demo.js`'s own redirect-back-to-`/?cookieless=1` logic — looping forever. Measured directly: `nav /?cookieless=1 -> 200`, `xhr /api/dashboard -> 401`, `dashboard.html`'s 401 handler sends the browser to `/login`, which sends it back. This is the exact audience the whole demo's mobile/LinkedIn-in-app-browser design decision names as primary.
- **Why parked:** This is the SDD process's fix-wave cap: one fix dispatch + one scoped re-review is the process's allowed response to a final-review finding, and this diff already used that slot. Per the `subagent-driven-development` skill, a second automated fix wave is not dispatched — residual load-bearing findings surface to the human instead. **This is NOT a "nice to fix later" item** — it blocks calling the branch ready for cookie-hostile visitors, which is the demo's stated primary use case. Surfaced prominently in the session's final report to the human, not treated as routine deferred debt.
- **Follow-up:** The bypass needs to survive past the first navigation without relying on a query param that XHRs don't carry — e.g. `demo.js` appending `cookieless=1` to same-origin fetches once the page has established it's in cookie-hostile mode, or the server treating a `Referer`/`Sec-Fetch-Site` header from a cookieless-marked page as sufficient. The existing test that would have caught this (`test_cookieless_request_still_reaches_a_login_gated_route`) was edited to add `?cookieless=1` to match the new code rather than continuing to assert the original bare-path behavior — a real end-to-end test driving a client that never sends `?cookieless=1` past the first request is needed before this can be called fixed.
...
- **What:** The fix wave correctly made `login.html`'s own post-login redirect preserve `?provider=`, and made `demo.js` only fire the bootstrap trigger on the dashboard page (not pre-auth). But the design spec's actual entry point for a logged-out reader is `GET /?provider=X` (the wizard's "Finish & Deploy" redirect target) — and `main.py`'s `SessionRequired` dependency, on an unauthenticated `GET /`, returns a hardcoded `RedirectResponse("/login")` with no query string at all. Measured: `GET /?provider=vertex -> 303 Location: /login` (param gone). So on the primary documented path, the reader's provider choice is lost before `login.html`'s own (now-correct) redirect logic ever runs, and the review silently falls back to the default provider.
- **Why parked:** Same fix-wave cap as above — this diff already used its one allowed fix+re-review slot.
- **Follow-up:** `main.py`'s `SessionRequired`/`require_session` redirect (shared with the real, non-demo dashboard) needs a demo-specific override that preserves the incoming query string on the `/`→`/login` redirect, without changing the real dashboard's redirect behavior. This wasn't caught by any test because the new coverage only checked the two shipped assets' content, not the `/` → `/login` hop itself.
