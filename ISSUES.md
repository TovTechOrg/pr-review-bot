...
- **Follow-up:** add a comment to `EXPECTED_COLUMNS` (and/or to `RUNTIME_CONFIG_COLUMNS`/`SLOT_CONFIG_COLUMNS`'s own docstrings) cross-referencing the other, so a column addition's failure here is expected rather than surprising, and the touchpoint count agrees across the code and this repo's `CLAUDE.md`.
